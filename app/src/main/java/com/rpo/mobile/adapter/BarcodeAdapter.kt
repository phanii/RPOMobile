...
class BarcodeAdapter(private val barcodeList: ArrayList<BarcodeBean>?) : RecyclerView.Adapter<BarcodeAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        /*  val count = Collections.frequency(barcodeList, bbean)
          if (barcodeList != null) {
              if (barcodeList.contains(bbean)) {
                  //Log.d("barcode exists ", ": ${bbean?.barcode} ---${Collections.frequency(barcodeList, bbean)} --- $position");
                  if (count > 1) {
                      //    barcodeList.removeAt(position)
                      // notifyDataSetChanged()

                      // Log.d(TAG, "Couont greater than One values : ${Gson().toJson(bbean)}");
